...
class MemberAllBookOnLoansTest {
    ...
    void 貸出制限の判定ができる(MemberType memberType, String loanDate1, String loanDate2, LoanRestrictions expected) {
        ...
        bookOnLoans.add(new BookOnLoan(null, member, null, new LoanDate(Date.from(loanDate1))));

        if (loanDate2 != null) {
            bookOnLoans.add(new BookOnLoan(null, member, null, new LoanDate(Date.from(loanDate2))));
        }
