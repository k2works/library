...
import library.domain.model.bookonloan.LoadingOfBook;
...
public class BookOnLoanRegisterController {
    ...
    String register(@Validated @ModelAttribute("loaningOfBookForm") LoaningOfBookForm loaningOfBookForm, BindingResult result, RedirectAttributes attributes) {
        ...
        LoadingOfBook loadingOfBook = new LoadingOfBook(member, bookCollection, loaningOfBookForm.loanDate);

        BookOnLoanValidResult valid = bookOnLoanRegisterCoordinator.isValid(loadingOfBook);
        ...
        bookOnLoanRecordService.registerBookOnLoan(loadingOfBook);

        attributes.addAttribute("memberNumber", loadingOfBook.member().memberNumber());
