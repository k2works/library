...
public class BookOnLoanRegisterCoordinator {
    ...
    public BookOnLoanValidResult isValid(LoadingOfBook loadingOfBook) {
        if (loadingOfBook.bookCollection().bookCollectionStatus() == BookCollectionStatus.貸出中) {
            return BookOnLoanValidResult.貸出中の蔵書;
        }

        MemberAllBookOnLoans memberAllBookOnLoans = bookOnLoanQueryService.findMemberAllBookOnLoans(loadingOfBook.member());
